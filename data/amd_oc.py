...
class ProgressThread(Thread):  #install thread
	def run(self):
		...
		progrtext = "Загрузка ROC-smi и OhGodATool..."
		work = director + "/amd-oc-install.bash --download"
		runcmd(work)
		progr = 0.25
		
		progrtext = "Установка ROC-smi и OhGodATool..."
		...
		if oldkernel is True:
			progrtext = "Обновление ядра с помощью UKUU..."
...
class MainWindow(Gtk.Window):
	def __init__(self):
		...
		self.button = Gtk.Button(label="Выбрать")
		...
		self.setmclk = Gtk.Button(label="Применить")
		...
		self.setvolt = Gtk.Button(label="Применить")
		...
		self.setfan = Gtk.Button(label="Применить")
		...
		self.applyall = Gtk.Button(label="Применить все")
		self.applyall.connect("clicked", self.on_applyall_clicked)
		sbox.pack_start(self.applyall, True, True, 0)
		self.write = Gtk.Button(label="Записать эти настройки в скрипт автозапуска")
	...
	def on_applyall_clicked(self, button):
		...
		if self.power.get_text().isdigit() and self.cclk.get_text().isdigit() and self.mclk.get_text().isdigit() and self.fan.get_text().isdigit() and self.volt.get_text().isdigit() and not selectedgpu == None:
			...
			Dialog("Ошибка!", "Введены неправильные данные или GPU не выбран!")
	def on_write_clicked(self, button):
		...
		if self.power.get_text().isdigit() and self.cclk.get_text().isdigit() and self.mclk.get_text().isdigit() and self.fan.get_text().isdigit() and self.volt.get_text().isdigit() and not selectedgpu == None:
			...
			runcmd(work)
			Dialog("OK", "Эти настройки для GPU %s установлены на автозапуск." % str(selectedgpu))
		else:
			Dialog("Ошибка!", "Введены неправильные данные или GPU не выбран!")
...
class InstallWindow(Gtk.Window):
	def __init__(self):
		global oldkernel
		Gtk.Window.__init__(self, title="Обновление системы для разгона AMD")
		self.set_default_size(500, 150)
		label = Gtk.Label()
		if oldkernel is True:
			label.set_markup("<big>Hello!</big>\n"
							"Версия ядра Linux: " + pp("uname -r").read() + ""
							"Оно будет обновлено до 4.17 потому, что наблюдаются проблемы с\n"
							"разгоном на ранних ядрах. ПО для разгона также будет установлено."
							)  #window text
		else:
			label.set_markup("<big>Hello!</big>\n"
							"Версия ядра Linux: " + pp("uname -r").read() + ""
							"Будет установлено только ПО для разгона."
							)  #window text
		box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
		box.set_homogeneous(False)
		box.pack_start(label, True, True, 0)
	...
	def on_timeout(self, user_data):
		...
		if reboot == True:
			Dialog("Перезагрузка", "Нажмите OK для перезагрузки.")
			pp("/sbin/reboot")
			quit()
		return True
