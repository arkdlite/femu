...
from apt import Cache as aptCache
...
class ProgressThread(Thread):  #install thread
    def run(self):
        global progress, progress_text, oldkernel, reboot
        progress = 0.01

        progress_text = "Downloading ROC-smi and OhGodATool..."
        work = config.DIR + "/amd-oc-install.bash --download"
        runcmd(work)
        progress = 0.8

        runcmd("systemctl enable amdgpu-oc")

        progress = 0.75

        if oldkernel is True:
            progress_text = "Upgrading kernel with UKUU tool..."
            work = config.DIR + "/amd-oc-install.bash --kernel"
            runcmd(work)
            reboot = True
...
if not '4.18' in pp("uname -r").read():
    ...
    if amdfound is False:
        Dialog("Error!", "AMD GPUs not found!")
        quit()
    print("test")
    window = MainWindow()
